defmodule QuizTest do
  ...
  describe "when a quiz has two templates" do
    ...
    @doc """
    select_question picks questions at random, so we first test to make sure
    that first and second templates are different and then we assert that on
    reset, the picked template is again one of the only 2 options available
    """
